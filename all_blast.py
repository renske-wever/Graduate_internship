...
def get_genes(genefile):
    ...
    for line in gene_file:
        ...
        STRAND = int(line[4])
        DESCRIPTION = line[1]
        gene_dict[gene_ID] = [START,STOP,STRAND,DESCRIPTION]
    ...
    for sequence_key in sequence_dict:
        ...
        for line in mutation_file:
            ...
            if ID == sequence_key:
                ...
                mutation_dict[mutation_ID] = ID,sequence,POS,ORG,ALT
                mutation_count += 1
...
def mutation_identification(mutation_dict,gene_dict):
    ...
    for key in mutation_dict:
        for key_2 in gene_dict:
            if mutation_dict[key][0] in key_2:
                description = gene_dict[key_2][3]
                sequence = mutation_dict[key][1]
                pos = int(mutation_dict[key][2])
                start = int(gene_dict[key_2][0])
                stop = int(gene_dict[key_2][1])
                strand = gene_dict[key_2][2]
                ...
                if len(org) == 1 and "," in alt:
                    ...
                    if pos in gen_range and "hypothetical protein" not in description:
                        ...
                        if strand == 1:
                            positive +=2
                            data_dict[key] = sequence,pos,org,alt,description,key_2
                        if strand == -1:
                            negative +=2
...
def get_Codons(data_dict):
    ...
    for key in data_dict:
        ...
        GENE_NAME = data_dict[key][4]
        GENE_ID = data_dict[key][5]
        ...
        if POS in range (0,len(sequence),3):
            ...
            if len(ORG) == 1 and "," in ALT:
                ...
                for i in alterations:
                    ...
                    if str(REF_AA) == str(ALT_AA):
                        AA_dict[new_key] = str(REF_AA),str(ALT_AA),"synonymous",GENE_NAME,GENE_ID
                    if str(REF_AA) != str(ALT_AA):
                        AA_dict[new_key] = str(REF_AA),str(ALT_AA),"non-synonymous",GENE_NAME,GENE_ID
                    count += 1
            ...
            if len(ORG) == 1 and len(ALT) == 1 and "," not in ALT:      
                ...
                ALT_AA = ALT_RNA.translate()
                if str(REF_AA) == str(ALT_AA):
                    AA_dict[key] = str(REF_AA),str(ALT_AA),"synonymous",GENE_NAME,GENE_ID
                if str(REF_AA) != str(ALT_AA):
                    AA_dict[key] = str(REF_AA),str(ALT_AA),"non-synonymous",GENE_NAME,GENE_ID
...
def write_output_file(output_dict,output_file):
    ...
    output.write("ID" + "\t" + "ORG AA" + "\t" + " ALT AA" + "\t" + "synonymous/non-synonymous" + "\t" + "gene name" + "\t" + "gene ID" + "\n" )
    for key in output_dict:
        output.write(key + "\t" + output_dict[key][0] + "\t" + output_dict[key][1]  + "\t" + output_dict[key][2] + "\t" + output_dict[key][3] + "\t" + output_dict[key][4] + "\n")
    output.close()
